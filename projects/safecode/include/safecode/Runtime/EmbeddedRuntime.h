...
extern RangeTree *ExternalObjects;

NAMESPACE_SC_BEGIN

typedef struct EmbeddedPoolTy_t {
  RangeTree Objects;
} EmbeddedPoolTy;

NAMESPACE_SC_END

#define PPOOL NAMESPACE_SC::EmbeddedPoolTy*
/*#define TAG unsigned */

extern "C" {
  void  pool_init_runtime(unsigned Dangling, unsigned RewriteOOB, unsigned Terminate);
  void  pool_register_global (PPOOL Pool, void * p, unsigned size);
  void  pool_register_stack (PPOOL Pool, void * allocaptr, unsigned NumBytes);
  void  pool_unregister_stack (PPOOL Pool, void * allocaptr);
  void* boundscheckui (PPOOL Pool, void * Source, void * Dest);
  void* exactcheck2 (const char *source, const char *base, const char *result, unsigned size);
  void  pool_register(PPOOL Pool, void *p, unsigned size);
}

extern "C" {
  /* Most of these functions are unused and possibly unimplemented (currently)
   * and/or declared elsewhere. This should probably be cleaned up later */
  /*void pool_init_runtime(unsigned Dangling,
                         unsigned RewriteOOB,
                         unsigned Terminate);*/

  void poolcheck(PPOOL Pool, void *Node);
  void poolcheckui(PPOOL Pool, void *Node);
  /*void poolcheck_debug (PPOOL, void * Node, TAG, SRC_INFO);
  void poolcheckui_debug (PPOOL, void * Node, TAG, SRC_INFO);*/

  void poolcheckalign(PPOOL Pool, void *Node, unsigned Offset);
  /*void poolcheckalign_debug (PPOOL, void *Node, unsigned Offset, TAG, SRC_INFO);*/

  void * boundscheck   (PPOOL Pool, void * Source, void * Dest);
