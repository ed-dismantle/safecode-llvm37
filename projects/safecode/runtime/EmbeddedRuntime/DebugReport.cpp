...
void
OutOfBoundsViolation::print() const {
  //
  // Print out the regular error information.
  //
  ViolationInfo::print();
  ...
  printf("= Object start                          :\t"
     "0x%lx\n"
     "= Object length                         :\t"
     "0x%lx\n", this->objStart, this->objLen);
}

void
AlignmentViolation::print() const {
  //
  // Print out the regular error information.
  //
  OutOfBoundsViolation::print();
...
WriteOOBViolation::print() const {
  ...
  //
  if (-1 != this->srcSize) {
    printf("= Source size (in bytes)                :\t"
       "%lu\n", this->srcSize);;
  }

  printf("= Destination size (in bytes)           :\t"
     "%lu\n", this->dstSize);

  if (-1 != this->copied) {
    printf("= Number of bytes copied                :\t"
       "%lu\n", this->copied);
  }
}

void
CStdLibViolation::print() const {
  //
  // Print out the regular error information.
  //
  ViolationInfo::print();
