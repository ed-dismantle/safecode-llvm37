...
namespace
{
  ...
  find_object(call_info *c, pointer_info *p)
  {
    ...
    EmbeddedPoolTy *pool = (EmbeddedPoolTy *) p->pool;
    ...
    {
      printf("Object not found in pool!\n");
  ...
  write_check(call_info *c, const options_t options, pointer_info *p, size_t n)
  {
    ...
    {
      printf("The destination of the write isn't a valid pointer!\n");
    ...
    if (p->flags & NULL_PTR)
    {
      printf("Writing into a NULL pointer!\n");
      c_library_error(c, "va_arg");
      return false;
    }
  ...
  varg_check(call_info *c, const options_t options, unsigned pos)
  {
    ...
    else if (pos > c->vargc)
    {
      if (c->vargc == 1)
      {
        printf("Attempting to access argument %u"
          " but there is only 1 argument!\n", pos);
      } 
      else
      {
        printf("Attempting to access argument %u"
          " but there are only %u arguments!\n", pos, c->vargc);
      }
      c_library_error(c, "va_arg");
      return true;
    }
              ...
              // map<void *, pointer_info *> &mp)
  {
    ...
    pointer_info *result = (pointer_info*)mp.find(ptr);
    if (result != NULL) return result;

    // If not found, create a new entry for the pointer...
    pointer_info *p = new pointer_info();
    p->ptr = ptr;
    p->pool = 0;
    p->flags = 0; // Don't add any flags.
    mp.insert((void*)ptr, (void*)p);
    return p;
  }
}
