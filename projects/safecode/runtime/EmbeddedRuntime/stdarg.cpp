...
typedef cmap VaListToArgIndexMap;
typedef cset VaListSet;
typedef struct {
  VaListSet referrers;
  cvector pointerList;
} ArgListEntry;
...
static inline void clearVaList(va_list *ap) {
  if (!vaListRegistrations().count((void*)ap))
    return;
  unsigned idx = (unsigned)vaListRegistrations().find((void*)ap);
  vaListRegistrations().erase((void*)ap);
  ((ArgListEntry*)argLists().at(idx))->referrers.erase((void*)ap);
}
...
void __sc_varegister(va_list ap, uint32_t id) {
  ...
  vaListRegistrations().at((void*)&ap, (void*)id);
  ((ArgListEntry*)argLists().at(id))->referrers.insert((void*)&ap);
}
...
void __sc_vacopyregister(va_list dest, va_list src) {
  ...
  if (!vaListRegistrations().count((void*)&src))
    return;
  // Remove all references of the destination list.
  clearVaList(&dest);
  // Register the destination list with the same information as the source list.
  unsigned idx = vaListRegistrations().at((void*)&dest, vaListRegistrations().find((void*)&src));
  ((ArgListEntry*)argLists().at(idx))->referrers.insert((void*)&dest);
}

// Add a new entry to the lists of pointer arguments.
void __sc_vacallregister(void *func, uint32_t argc, ...) {
  ArgListEntry* end = (ArgListEntry*)malloc(sizeof(ArgListEntry));;//(ArgListEntry*)argLists().back();
  argLists().push_back((void*)end);
...
void __sc_vacallunregister() {
  ArgListEntry *last = (ArgListEntry*)argLists().back();
  // Remove each va_list associated with this list from the hash table.
  for (int i = 0; i < last->referrers.size(); ++i)
    vaListRegistrations().erase(last->referrers.get(i));
  // Now remove the last entry altogether.
  free((ArgListEntry*)argLists().pop_back());
}
...
build_call_info(call_info *&result, va_list ap) { //, TAG, SRC_INFO) {
  ...
  unsigned idx = (unsigned)vaListRegistrations().find(&ap);
  if (idx == 0) {//vaListRegistrations().end()) {
  ...
  // whitelist as specified in the corresponding index.
    ...
    cvector &pointerList = ((ArgListEntry*)argLists().at(index))->pointerList;
    size_t wl_size = pointerList.size();
    ...
    if (result != 0) {
      ...
      for (unsigned i = 0; i < wl_size; ++i)
        result->whitelist[i] = pointerList.at(i);
...
static inline void
create_wrapper(pointer_info &dest, void *p, EmbeddedPoolTy *pool, bool complete) {
  dest.ptr = p;
  dest.pool = (void *) pool;
...
//
int pool_vprintf(EmbeddedPoolTy *fmtPool,
                 char *fmt,
                 va_list ap,
                 bv_t complete) {
...
//   number on error.
//
/*int pool_vprintf_debug(EmbeddedPoolTy *fmtPool,
                       char *fmt,
                       va_list ap,
                       bv_t complete,
                       TAG,
...
//
int pool_vfprintf(EmbeddedPoolTy *fPool,
                  EmbeddedPoolTy *fmtPool,
                  void *fil,
                  char *fmt,
                  va_list ap,
                  bv_t complete) {
...
//   number on error.
//
/*int pool_vfprintf_debug(EmbeddedPoolTy *fPool,
                        EmbeddedPoolTy *fmtPool,
                        void *fil,
                        char *fmt,
                        va_list ap,
...
//
int pool_vsprintf(EmbeddedPoolTy *strPool,
                  EmbeddedPoolTy *fmtPool,
                  char *str,
                  char *fmt,
                  va_list ap,
...
//   number on error.
//
/*int pool_vsprintf_debug(EmbeddedPoolTy *strPool,
                        EmbeddedPoolTy *fmtPool,
                        char *str,
                        char *fmt,
                        va_list ap,
...
//
int pool_vsnprintf(EmbeddedPoolTy *strPool,
                   EmbeddedPoolTy *fmtPool,
                   char *str,
                   char *fmt,
                   size_t n,
...
//   number on error.
//
/*int pool_vsnprintf_debug(EmbeddedPoolTy *strPool,
                         EmbeddedPoolTy *fmtPool,
                         char *str,
                         char *fmt,
                         size_t n,
...
//
int pool_vscanf(EmbeddedPoolTy *fmtPool,
                char *fmt,
                va_list ap,
                bv_t complete) {
...
//
/*int pool_vscanf_debug(EmbeddedPoolTy *fmtPool,
                      char *fmt,
                      va_list ap,
                      bv_t complete,
                      TAG,
...
//
int pool_vsscanf(EmbeddedPoolTy *strPool,
                 EmbeddedPoolTy *fmtPool,
                 char *str,
                 char *fmt,
                 va_list ap,
...
//
/*int pool_vsscanf_debug(EmbeddedPoolTy *strPool,
                       EmbeddedPoolTy *fmtPool,
                       char *str,
                       char *fmt,
                       va_list ap,
...
//
int pool_vfscanf(EmbeddedPoolTy *filPool,
                 EmbeddedPoolTy *fmtPool,
                 void *fil,
                 char *fmt,
                 va_list ap,
...
//
/*int pool_vfscanf_debug(EmbeddedPoolTy *filPool,
                       EmbeddedPoolTy *fmtPool,
                       void *fil,
                       char *fmt,
                       va_list ap,
...
//
/*void pool_vsyslog(EmbeddedPoolTy *fmtPool,
                  char *fmt,
                  int priority,
                  va_list ap,
                  bv_t complete) {
...
//   number on error.
//
/*void pool_vsyslog_debug(EmbeddedPoolTy *fmtPool,
                        char *fmt,
                        int priority,
                        va_list ap,
                        bv_t complete,
