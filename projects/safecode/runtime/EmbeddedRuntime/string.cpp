...
//
void
poolcheckstr (EmbeddedPoolTy * Pool, const char * str) {
  if (str == NULL) return;
  validStringCheck (str, Pool, true, "Generic");//, DEFAULT_SRC_INFO);
}
/*
void
poolcheckstr_debug (EmbeddedPoolTy * Pool, char * str, TAG, SRC_INFO) {
  if (str == NULL) return;
  validStringCheck (str, Pool, true, "Generic", SRC_INFO_ARGS);
}*/

void
poolcheckstrui (EmbeddedPoolTy * Pool, char * str) {
  if (str == NULL) return;
  validStringCheck (str, Pool, false, "Generic");//, DEFAULT_SRC_INFO);
}
/*
void
poolcheckstrui_debug (EmbeddedPoolTy * Pool, char * str, TAG, SRC_INFO) {
  if (str == NULL) return;
...
//
void *
pool_memccpy(EmbeddedPoolTy *dstPool,
             EmbeddedPoolTy *srcPool,
             void *dst,
             void *src,
             char c,
             size_t n,
  ...
  if (!(dstFound = pool_find(dstPool, dst, dstBegin, dstEnd)) && dstComplete) {
    printf("Memory object not found in pool!\n");
  ...
  if (srcFound) {
    ...
    if (bytesToCopy > srcSize) {
      printf("Cannot copy more bytes than the size of the source!\n");
      OOB_VIOLATION(src, srcPool, src, bytesToCopy);//, SRC_INFO_ARGS);
    }
    if (dstFound) {
      dstSize = byte_range(dst, dstEnd);
      if (bytesToCopy > dstSize) {
        printf("Cannot copy more bytes than the size of the destination!\n");
      ...
      if (isOverlapped(dst, dstLimit, src, srcLimit)) {
        printf("Input memory objects overlap each other!\n");
...
//
/*void *
pool_memccpy_debug(EmbeddedPoolTy *dstPool, 
                   EmbeddedPoolTy *srcPool, 
                   void *dst, 
                   void *src, 
                   int c,
...
//
void *
pool_memchr(EmbeddedPoolTy *strPool, 
            void *str, 
            int c, 
            size_t n,
            const uint8_t complete) {
  ...
  if (strFound) {
    strSize = min(byte_range(str, strEnd), n);
    stop    = memchr(str, c, strSize);
    if (stop != NULL)
      return stop;
    else if (n > strSize) {
      printf("memchr() reads past the end of the memory object!\n");
...
//
/*void *
pool_memchr_debug(EmbeddedPoolTy *strPool, 
                  void *str,
                  int c, 
                  size_t n,
                  const uint8_t complete,
...
//
int
pool_memcmp(EmbeddedPoolTy *s1Pool,
            EmbeddedPoolTy *s2Pool,
            void *s1,
            void *s2,
            size_t num,
            const uint8_t complete) {
  ...
  // of bounds.
  size_t stop = min(num, min(s1Size, s2Size));
  for (p = 0; p < stop; p++)
    if (cs1[p] != cs2[p])
      return cs1[p] - cs2[p];
  if (p == num)
    return 0;
  if (s1Found && p == s1Size) {
    printf("memcmp() reads beyond the end of bytestring 1!\n");
    OOB_VIOLATION(s1, s1Pool, s1, s1Size + 1);//, SRC_INFO_ARGS);
  }
  if (s2Found && p == s2Size) {
    printf("memcmp() reads beyond the end of bytestring 2!\n");
...
//
/*int
pool_memcmp_debug(EmbeddedPoolTy *s1Pool,
                  EmbeddedPoolTy *s2Pool, 
                  void *s1, 
                  void *s2,
                  size_t num,
...
//
void *
pool_memcpy(EmbeddedPoolTy *dstPool, 
            EmbeddedPoolTy *srcPool, 
            void *dst, 
            void *src, 
            size_t n,
  ...
  if (!(dstFound = pool_find(dstPool, dst, dstBegin, dstEnd)) && dstComplete) {
    printf("Destination object not found in pool!\n");
    LOAD_STORE_VIOLATION(dst, dstPool);//, SRC_INFO_ARGS);
  }
  if (!(srcFound = pool_find(srcPool, src, srcBegin, srcEnd)) && srcComplete) {
    printf("Source object not found in pool!\n");
    LOAD_STORE_VIOLATION(src, srcPool);//, SRC_INFO_ARGS);
  }
  // Calculate the maximum number of bytes to copy.
  dstSize = byte_range(dst, dstEnd);
  ...
  if (srcFound && n > srcSize) {
    printf("memcpy() reads beyond the source object's boundaries!\n");
    OOB_VIOLATION(src, srcPool, src, n);//, SRC_INFO_ARGS);
  }
  if (dstFound && n > dstSize) {
    printf("memcpy() writes beyond the destination object's boundaries!\n");
...
//
/*void *
pool_memcpy_debug(EmbeddedPoolTy *dstPool, 
                  EmbeddedPoolTy *srcPool, 
                  void *dst, 
                  void *src, 
                  size_t n,
...
//
void *
pool_memmove(EmbeddedPoolTy *dstPool, 
             EmbeddedPoolTy *srcPool, 
             void *dst, 
             void *src, 
             size_t n,
  ...
  srcSize = byte_range(src, srcEnd);
  if (srcFound && n > srcSize) {
    printf("memmove() reads beyond the end of the source bytestring!\n");
    OOB_VIOLATION(src, srcPool, src, n);//, SRC_INFO_ARGS);
  }
  if (dstFound && n > dstSize) {
    printf("memmove() write beyond the end of the destination bytestring!\n");
...
//
/*void *
pool_memmove_debug(EmbeddedPoolTy *dstPool, 
                   EmbeddedPoolTy *srcPool, 
                   void *dst, 
                   void *src, 
                   size_t n,
...
//
void *pool_memset(EmbeddedPoolTy *sPool,
                  void *s,
                  int c,
                  size_t n,
                  const uint8_t complete) {
  ...
  if (sFound && n > size) {
    printf("memset() writes beyond the end of the destination object!\n");
...
//
/*void *
pool_memset_debug(EmbeddedPoolTy *sPool, 
                  void *s, 
                  int c, 
                  size_t n,
                  const uint8_t complete,
...
//
char *
pool_strcat(EmbeddedPoolTy *dstPool,
            EmbeddedPoolTy *srcPool,
            char *dst,
            char *src,
  ...
  if (!(dstFound = pool_find(dstPool, dst, dstBegin, dstEnd)) && dstComplete) {
    printf("Destination string not found in pool\n");
    LOAD_STORE_VIOLATION(dstBegin, dstPool);//, SRC_INFO_ARGS);
  }
  if (!(srcFound = pool_find(srcPool, src, srcBegin, srcEnd)) && srcComplete) {
    printf("Source string not found in pool!\n");
  ...
  if (dstTerminated && srcTerminated) {
    ...
    if (catLen > maxLen) {
      printf("Concatenation violated destination bounds!\n");
    ...
    if (&dst[dstLen] == &src[srcLen]) {
      printf("Concatenating overlapping strings is undefined\n");
      C_LIBRARY_VIOLATION(dst, dstPool, "strcat");//, SRC_INFO_ARGS);
    }
...
//
/*char *
pool_strcat_debug(EmbeddedPoolTy *dstPool,
                  EmbeddedPoolTy *srcPool,
                  char *dst,
                  char *src,
                  const uint8_t complete,
...
//
char *
pool_strchr(EmbeddedPoolTy *sPool, char *s, int c, const uint8_t complete) {
...
//
/*char *
pool_strchr_debug(EmbeddedPoolTy *sPool,
                  char *s,
                  int c,
                  const uint8_t complete,
                  TAG,
...
//
int
pool_strcmp(EmbeddedPoolTy *s1Pool,
            EmbeddedPoolTy *s2Pool,
            char *s1,
            char *s2,
            const uint8_t complete){
...
//
/*int
pool_strcmp_debug(EmbeddedPoolTy *s1Pool,
                  EmbeddedPoolTy *s2Pool,
                  char *s1,
                  char *s2,
                  const uint8_t complete,
...
//
int
pool_strcoll(EmbeddedPoolTy *s1Pool,
             EmbeddedPoolTy *s2Pool, 
             char *s1, 
             char *s2,
             const uint8_t complete){
...
//
/*int
pool_strcoll_debug(EmbeddedPoolTy *s1Pool,
                   EmbeddedPoolTy *s2Pool, 
                   char *s1, 
                   char *s2,
                   const uint8_t complete,
...
//
char *
pool_strcpy(EmbeddedPoolTy *dstPool,
            EmbeddedPoolTy *srcPool,
            char *dst,
            char *src,
            const uint8_t complete) {
  ...
  if (srcFound && !(srcTerminated = isTerminated(src, srcEnd, srcLen))) {
    printf("Source string is not terminated within object bounds!\n");
  ...
  if (dstFound) {
    ...
    if (srcTerminated) {
      ...
      if (srcLen > dstMax) {
        printf("strcpy() writes beyond the end of the destination object!\n");
      ...
      if (isOverlapped(dst, dstEdge, src, srcEdge)) {
        printf("Memory objects in call to strcpy() overlap each other!\n");
...
//
/*char *
pool_strcpy_debug(EmbeddedPoolTy *dstPool,
                  EmbeddedPoolTy *srcPool,
                  char *dst,
                  char *src,
                  const uint8_t complete,
...
//
size_t pool_strcspn(EmbeddedPoolTy *str1Pool,
                    EmbeddedPoolTy *str2Pool,
                    char *str1,
                    char *str2,
                    const uint8_t complete) {
...
//
/*size_t
pool_strcspn_debug(EmbeddedPoolTy *str1Pool,
                   EmbeddedPoolTy *str2Pool, 
                   char *str1, 
                   char *str2,
                   const uint8_t complete,
...
//
size_t
pool_strlen(EmbeddedPoolTy *strPool,
            char *str,
            const uint8_t complete) {
  const bool strComplete = ARG1_COMPLETE(complete);
  bool strFound;
  ...
  if (!(strFound = pool_find(strPool, str, strBegin, strEnd)) && strComplete) {
    printf("Object for string not found in pool!\n");
    LOAD_STORE_VIOLATION(str, strPool);//, SRC_INFO_ARGS);
  }
  if (strFound) {
    if (!isTerminated(str, strEnd, len)) {
      printf("Input string not terminated within object boundaries!\n");
...
//
/*size_t
pool_strlen_debug(EmbeddedPoolTy *strPool, 
                  char *str, 
                  const uint8_t complete, 
                  TAG, 
                  SRC_INFO) {
...
//
char *
pool_strncat(EmbeddedPoolTy *dstPool,
             EmbeddedPoolTy *srcPool,
             char *dst,
             char *src,
             size_t n,
  ...
  if (dstFound && !(dstTerminated = isTerminated(dst, dstEnd, dstLen))) {
    printf("String not terminated within bounds\n");
  ...
  if (srcFound && !srcTerminated && byte_range(src, srcEnd) < n) {
    printf("strncat() reads beyond the boundaries of the source object!\n");
  ...
  if (srcFound && dstTerminated) {
    ...
    // src.
    // Otherwise this is n.
    srcAmt = srcTerminated ? min(srcLen, n) : n;
    ...
    if (dst < &src[srcAmt] && &src[srcAmt] <= &dst[dstLen]) {
      printf("Concatenating overlapping objects is undefined\n");
      C_LIBRARY_VIOLATION(dst, dstPool, "strncat");//, SRC_INFO_ARGS);
    }
...
//
/*char *
pool_strncat_debug(EmbeddedPoolTy *dstPool,
                   EmbeddedPoolTy *srcPool,
                   char *dst,
                   char *src,
                   size_t n,
...
//
int
pool_strncmp(EmbeddedPoolTy *s1Pool,
             EmbeddedPoolTy *s2Pool,
             char *s1,
             char *s2,
             size_t n,
  ...
  if (p == n)
    return 0;
  if (s1Found && p == s1Size) {
    printf("strncmp() reads beyond the end of string 1!\n");
    OOB_VIOLATION(s1, s1Pool, s1, s1Size + 1);//, SRC_INFO_ARGS);
  }
  if (s2Found && p == s2Size) {
    printf("strncmp() reads beyond the end of string 2!\n");
    OOB_VIOLATION(s2, s2Pool, s2, s2Size + 1);//, SRC_INFO_ARGS);
  }
...
//
/*int
pool_strncmp_debug(EmbeddedPoolTy *s1Pool,
                   EmbeddedPoolTy *s2Pool, 
                   char *s1, 
                   char *s2,
                   size_t n,
...
//
char *
pool_strncpy(EmbeddedPoolTy *dstPool,
             EmbeddedPoolTy *srcPool,
             char *dst,
             char *src,
             size_t n,
  ...
  if (srcFound) {
    ...
    if (n > srcSize && srcLen == srcSize) {
        printf("strncpy() reads source string out of bounds!\n");
        ...
        size_t srcRead = min(n, 1 + srcLen);
        void *dstEdge = (char *) dst + srcRead - 1;
        void *srcEdge = (char *) src + srcRead - 1;
        if (isOverlapped(dst, dstEdge, src, srcEdge)) {
          printf("The objects passed to strncpy() overlap!\n");
  ...
  if (dstFound) {
    dstSize = byte_range(dst, dstEnd);
    if (dstSize < n) {
      printf("strncpy() writes beyond end of destination object!\n");
      WRITE_VIOLATION(dst, dstPool, dstSize, n);//, SRC_INFO_ARGS);
    }
  }
...
//
/*char *
pool_strncpy_debug(EmbeddedPoolTy *dstPool,
                   EmbeddedPoolTy *srcPool,
                   char *dst, 
                   char *src,
                   size_t n, 
...
//
char *
pool_strpbrk(EmbeddedPoolTy *sPool,
             EmbeddedPoolTy *aPool,
             char *s,
             char *a,
             const uint8_t complete) {
  const bool sComplete = ARG1_COMPLETE(complete);
...
//
/*char *
pool_strpbrk_debug(EmbeddedPoolTy *sPool,
                   EmbeddedPoolTy *aPool,
                   char *s,
                   char *a,
                   const uint8_t complete,
...
//
char *
pool_strrchr(EmbeddedPoolTy *sPool,
             char *s,
             int c,
             const uint8_t complete) {
  const bool sComplete = ARG1_COMPLETE(complete);
...
//
/*char *
pool_strrchr_debug(EmbeddedPoolTy *sPool,
                   char *s,
                   int c,
                   const uint8_t complete,
                   TAG,
...
//
size_t
pool_strspn(EmbeddedPoolTy *str1Pool,
            EmbeddedPoolTy *str2Pool,
            char *str1,
            char *str2,
            const uint8_t complete) {
...
//
/*size_t
pool_strspn_debug(EmbeddedPoolTy *str1Pool,
                  EmbeddedPoolTy *str2Pool, 
                  char *str1, 
                  char *str2,
                  const uint8_t complete,
...
//
char *
pool_strstr(EmbeddedPoolTy *s1Pool,
            EmbeddedPoolTy *s2Pool,
            char *s1,
            char *s2,
            const uint8_t complete) {
...
//
/*char *
pool_strstr_debug(EmbeddedPoolTy *s1Pool,
                  EmbeddedPoolTy *s2Pool,
                  char *s1,
                  char *s2,
                  const uint8_t complete,
...
//
size_t
pool_strxfrm(EmbeddedPoolTy *dPool,
             EmbeddedPoolTy *sPool,
             char *d,
             char *s,
             size_t n,
             const uint8_t complete) {
  ...
  if (dFound && n > 0) {
    ...
    // string.
    size_t xfrmLen = min(strxfrm(NULL, s, 0), n - 1);
    size_t dSize   = byte_range(d, dEnd);
    if (xfrmLen + 1 > dSize) {
      printf("strxfrm() writes past the end of the destination object!\n");
...
//
/*size_t
pool_strxfrm_debug(EmbeddedPoolTy *dPool,
                   EmbeddedPoolTy *sPool,
                   char *d,
                   char *s,
                   size_t n,
...
//
void *
pool_mempcpy(EmbeddedPoolTy *dstPool,
             EmbeddedPoolTy *srcPool,
             void *dst,
             void *src,
             size_t n,
  ...
  srcSize = byte_range(src, srcEnd);
  if (dstFound && n > dstSize) {
    printf("mempcpy() writes past the end of the destination object!\n");
    WRITE_VIOLATION(dst, dstPool, dstSize, n);//, SRC_INFO_ARGS);
  }
  if (srcFound && n > srcSize) {
    printf("mempcpy() reads past the end of the source object!\n");
    OOB_VIOLATION(src, srcPool, src, n);//, SRC_INFO_ARGS);
  }
#if 0
  if (dstFound && srcFound) {
...
//
/*void *
pool_mempcpy_debug(EmbeddedPoolTy *dstPool, 
                   EmbeddedPoolTy *srcPool, 
                   void *dst, 
                   void *src, 
                   size_t n,
...
//
char *
pool_strcasestr(EmbeddedPoolTy *s1Pool,
                EmbeddedPoolTy *s2Pool,
                char *s1,
                char *s2,
                const uint8_t complete) {
...
//
/*char *
pool_strcasestr_debug(EmbeddedPoolTy *s1Pool,
                      EmbeddedPoolTy *s2Pool,
                      char *s1,
                      char *s2,
                      const uint8_t complete,
...
//
size_t
pool_strnlen(EmbeddedPoolTy *strPool,
             char *str,
             size_t maxlen,
             const uint8_t complete) {
  size_t safelen, len;
  ...
  if (strFound) {
    ...
    len     = _strnlen(str, min(maxlen, safelen));
    ...
    if (len == safelen && maxlen > safelen) {
      printf("strnlen() reads beyond the end of the input string's object!\n");
...
//
/*size_t
pool_strnlen_debug(EmbeddedPoolTy *strPool, 
                   char *str, 
                   size_t maxlen, 
                   const uint8_t complete, 
                   TAG, 
...
//
char *
pool_stpcpy(EmbeddedPoolTy *dstPool,
            EmbeddedPoolTy *srcPool,
            char *dst,
            char *src,
            const uint8_t complete) {
  ...
  if (!(dstFound = pool_find(dstPool, dst, dstBegin, dstEnd)) && dstComplete) {
    printf("Could not find destination object in pool!\n");
    LOAD_STORE_VIOLATION(dst, dstPool);//, SRC_INFO_ARGS);
  }
  if (!(srcFound = pool_find(srcPool, src, srcBegin, srcEnd)) && srcComplete) {
    printf("Could not find source object in pool\n");
    LOAD_STORE_VIOLATION(src, srcPool);//, SRC_INFO_ARGS);
  }
  // Check if source is terminated.
  if (srcFound && !isTerminated(src, srcEnd, srcLen)) {
    printf("Source string not terminated within bounds!\n");
  ...
  if (dstFound && srcFound) {
    ...
    if (isOverlapped(dst, &dst[srcLen], src, &src[srcLen])) {
      printf("Copying overlapping strings has undefined behavior!\n");
    ...
    if (srcLen > dstLen) {
      printf("Destination object too short to hold string!\n");
...
//
/*char *
pool_stpcpy_debug(EmbeddedPoolTy *dstPool,
                  EmbeddedPoolTy *srcPool,
                  char *dst,
                  char *src,
                  const uint8_t complete,
