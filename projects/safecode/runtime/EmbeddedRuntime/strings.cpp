...
//
int
pool_bcmp(safecode::EmbeddedPoolTy *aPool,
          safecode::EmbeddedPoolTy *bPool,
          void *a,
          void *b,
          size_t n,
  ...
  if (!(aFound = pool_find(aPool, (void *) a, aStart, aEnd)) && aComplete) {
    printf("Object for 1st argument to bcmp() not found in pool!\n");
    LOAD_STORE_VIOLATION(a, aPool);//, SRC_INFO_ARGS);
  }
  if (!(bFound = pool_find(bPool, (void *) b, bStart, bEnd)) && bComplete) {
    printf("Object for 2nd argument to bcmp() not found in pool!\n");
  ...
  size_t safelen = min(n, min(aSize, bSize));
  int    result  = bcmp(a, b, safelen);
  if (safelen == n)
    return result;
  else if (result == 0) {
    //err << "bcmp() reads beyond object boundaries!\n";
    printf("bcmp() reads beyond object boundaries!\n");
    if (aSize <= bSize)
...
//
/*int
pool_bcmp_debug(safecode::EmbeddedPoolTy *aPool,
                safecode::EmbeddedPoolTy *bPool,
                void *a,
                void *b,
                size_t n,
...
//
void
pool_bcopy(safecode::EmbeddedPoolTy *s1Pool,
           safecode::EmbeddedPoolTy *s2Pool,
           void *s1,
           void *s2,
           size_t n,
           const uint8_t complete) {
  ...
  if (s1Found && n > s1Bytes) {
    printf("bcopy() reads beyond the end of the source object!\n");
    OOB_VIOLATION(s1, s1Pool, s1, n);//, SRC_INFO_ARGS);
  }
  size_t s2Bytes = byte_range(s2, s2End);
  if (s2Found && n > s2Bytes) {
    printf("bcopy() writes beyond the end of the destination object!\n");
...
//
/*void
pool_bcopy_debug(safecode::EmbeddedPoolTy *s1Pool,
                 safecode::EmbeddedPoolTy *s2Pool,
                 void *s1,
                 void *s2,
                 size_t n,
...
//
void
pool_bzero(safecode::EmbeddedPoolTy *sPool,
           void *s,
           size_t n,
           const uint8_t complete) {
  ...
  if (sFound && n > sBytes) {
    ...
    printf("bzero() writes beyond the end of the destination memory object!\n");
...
//
/*void
pool_bzero_debug(safecode::EmbeddedPoolTy *sPool,
                 void *s,
                 size_t n,
                 const uint8_t complete,
                 TAG,
...
//
char *
pool_index(safecode::EmbeddedPoolTy *sPool,
           char *s,
           int c,
           const uint8_t complete) {
  const bool sComplete = ARG1_COMPLETE(complete);
...
//
/*char *
pool_index_debug(safecode::EmbeddedPoolTy *sPool,
                 char *s,
                 int c,
                 const uint8_t complete,
                 TAG,
...
//
char *
pool_rindex(safecode::EmbeddedPoolTy *sPool,
           char *s,
           int c,
           const uint8_t complete) {
  const bool sComplete = ARG1_COMPLETE(complete);
...
//
/*char *
pool_rindex_debug(safecode::EmbeddedPoolTy *sPool,
                  char *s,
                  int c,
                  const uint8_t complete,
                  TAG,
...
//
int
pool_strcasecmp(safecode::EmbeddedPoolTy *str1Pool,
                safecode::EmbeddedPoolTy *str2Pool, 
                char *str1, 
                char *str2,
                const uint8_t complete) {
...
//
/*int
pool_strcasecmp_debug(safecode::EmbeddedPoolTy *str1Pool,
                      safecode::EmbeddedPoolTy *str2Pool, 
                      char *str1, 
                      char *str2,
                      const uint8_t complete,
...
//
int
pool_strncasecmp(safecode::EmbeddedPoolTy *s1Pool,
                 safecode::EmbeddedPoolTy *s2Pool, 
                 char *s1, 
                 char *s2,
                 size_t n,
  ...
  if (!(s1Found = pool_find(s1Pool, s1, s1Start, s1End)) && s1Complete) {
    printf("Memory object containing string 1 not found in pool!\n");
    LOAD_STORE_VIOLATION(s1, s1Pool);//, SRC_INFO_ARGS);
  }
  if (!(s2Found = pool_find(s2Pool, s2, s2Start, s2End)) && s2Complete) {
    printf("Memory object containing string 2 not found in pool!\n");
    LOAD_STORE_VIOLATION(s2, s2Pool);//, SRC_INFO_ARGS);
  }
  ...
  // of bounds.
  size_t safe   = min(n, min(s1Safe, s2Safe));
  // Compare the strings safely.
  for (size_t i = 0; i < safe; ++i) {
    if (int diff = tolower(str1[i]) - tolower(str2[i]))
      return diff;
  ...
  if (safe == n) // Strings are equal up to the first n characters.
    ...
    printf("strncasecmp() reads beyond the end of string's object!\n");
    if (s1Safe <= s2Safe)
...
//
/*int
pool_strncasecmp_debug(safecode::EmbeddedPoolTy *s1Pool,
                       safecode::EmbeddedPoolTy *s2Pool, 
                       char *s1, 
                       char *s2,
                       size_t n,
